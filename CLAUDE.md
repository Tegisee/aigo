...
**🔴 P0 — 기능 블로커**
1. **ENV-2 → 온보딩 구글 로그인 UX 개편**: 구글 로그인 시 아이/부모 정보 미복원 → 온보딩 UX 개편으로 근본 해결 필요 (신규 사용자 경험 직결)
2. **ENV-1**: 기념일 추천상품 미동작 (여전히 안 됨) — 1.0.0 커밋과 git diff로 원인 재분석 필요
3. **BUG-12**: 쿠팡 공유 시 2번 튕김 (여전히 안 됨) — 1.0.0 커밋과 git diff로 원인 재분석 필요

**🟡 P1 — UX 버그**
- **BUG-30**: 병원명 키패드 가림 (완전 해결 안 됨)
- **BUG-33**: 병원명 길어지면 텍스트 잘림 → 접종일/병원명 레이아웃 개선 필요 (병원명 한 줄 아래로 분리)

**🟢 낮음**
- 육아정보 API 2단계 (L)

**다음 단계**: P0 1번(온보딩 구글 로그인 UX 개편) → ENV-1/BUG-12 재분석 → 로컬 빌드 → 비공개 테스트 제출
