...
### v1.0.4 vc15 테스트 결과 (04-12)
**✅ 완료 확인**
- ENV-1: 카테고리/기념일 추천상품 정상 동작
- BUG-23: 푸시 토큰 Firestore 저장 OK
- BUG-30/33: 병원명 2줄 표시 + 키패드 가림 해결

**⬜ 미해결**
- BUG-12: 쿠팡 튕김 여전 (지금이야 방식 복원에도 미해결)
- ENV-2: 구글 로그인 데이터 복원 미해결 (온보딩 첫화면 구글 복원 안 됨)

**🆕 신규 발견**
- BUG-34: 관심상품 다자녀 탭 전환 시 전체로 이동하는 버그
- BUG-35: 공유상품 있는 카테고리에서 1개만 표시되는 문제
- BUG-36: 접종 리스트만 등록 후 나중에 체크하는 기능 미구현

### 남은 TODO

**🔴 P0 — 미해결**
1. **ENV-2**: 온보딩 구글 로그인 플로우 재설계 (첫화면 복원 안 됨 → 근본 해결 필요)
2. **BUG-12**: 쿠팡 공유 튕김 재분석 (지금이야와 동일 코드인데도 튕김 → 다른 원인 존재)

**🟡 P1 — UX 버그**
- **BUG-34**: 관심상품 다자녀 탭 전환 시 전체로 이동
- **BUG-35**: 공유상품 카테고리에서 1개만 표시
- **BUG-36**: 접종 리스트 등록 후 나중에 체크 기능

**🟢 낮음**
- 육아정보 API 2단계 (L)

**다음 단계**: ENV-2 근본 해결 → BUG-12 재분석 → BUG-34/35 수정 → 로컬 빌드 → 비공개 테스트 제출
