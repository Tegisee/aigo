...
### v1.0.4 vc14 수정 내역 (04-12)
- ✅ ENV-2: 온보딩 구글 계정 복원 UX 추가 → 근본 해결
- ✅ BUG-33: 접종일/병원명 한 줄씩 분리 표시
- 🔧 BUG-12: 재수정 (link.coupang.com 허용 복원 + coupang scheme 쿼리 제거) → 테스트 대기
- 🔧 BUG-30: 재수정 (behavior="padding" + 하단 여유 공간) → 테스트 대기
- 🔧 ENV-1: EAS env plaintext 재등록 완료 → 테스트 대기

### 남은 TODO

**🔧 테스트 대기 (vc14)**
- **ENV-1**: EAS plaintext 재등록 → 카테고리 추천 동작 확인
- **BUG-12**: 쿠팡 공유 튕김 재수정 → 동작 확인
- **BUG-30**: 병원명 키패드 가림 재수정 → 동작 확인

**🟢 낮음**
- 육아정보 API 2단계 (L)

**다음 단계**: vc14 로컬 빌드 → 테스트 → 비공개 테스트 제출
