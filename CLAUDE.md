...
### v1.0.4 vc14 테스트 결과 (04-12)
**✅ 정상 확인**
- BUG-23: 푸시 토큰 Firestore 저장 OK → 완료
- ENV-2: 구글 계정 연동 후 아이 정보 복원 정상 → 완료

**⬜ 미해결**
- ENV-1: EAS plaintext 재등록에도 카테고리/기념일 추천상품 미동작 → 로컬 빌드에서 .env 읽히는지 직접 확인 필요
- BUG-12: 15초 버퍼링 후 쿠팡 튕김 여전 → 근본 원인 재분석 필요
- BUG-30/33: 병원명 키패드 가림 + 줄 안바뀜 → 재수정 필요

### 남은 TODO

**🔴 P0 — 미해결**
1. **ENV-1**: 카테고리/기념일 추천상품 미동작 — 로컬 빌드에서 .env가 Metro에 전달되는지 확인 필요
2. **BUG-12**: 쿠팡 공유 15초 버퍼링 후 튕김 — 근본 원인 재분석 필요
...
**다음 단계**: ENV-1 추가 분석 → BUG-12 근본 재분석 → BUG-30/33 재수정 → 로컬 빌드 → 비공개 테스트 제출
