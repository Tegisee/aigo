...
### v1.0.4 vc17 테스트 결과 (04-12)
**✅ 완료 확인**
- BUG-12: 쿠팡 튕김 최종 해결 (withAndroidQueries 제거)
- BUG-34: 관심상품 다자녀 탭 전환 버그 수정
- BUG-35: 카테고리 추천상품 공유상품+API 합치기

**🆕 신규 발견**
- BUG-37: 월령별 추천 카테고리 복수선택 해제 안 됨 (단일 선택으로 변경 필요)

**⬜ 미해결**
- ENV-2: 구글 로그인 데이터 복원 미해결 (온보딩 첫화면 구글 복원 안 됨)
- BUG-36: 접종 리스트 등록 후 나중에 체크 기능 미구현

### 남은 TODO

**🔴 P0 — 미해결**
- **ENV-2**: 온보딩 구글 로그인 플로우 재설계 (첫화면 복원 안 됨 → 근본 해결 필요)
...
**다음 단계**: ENV-2 근본 해결 → BUG-37 수정 → 로컬 빌드 → 비공개 테스트 제출
